...
# Remove lock file in case previous run was forcefully stopped删除锁文件，以防强制停止前一次运行
$(shell rm -f .screen)
...
# Support FIPS feature, armhf not supported yet支持FIPS功能，ARMHF尚未支持
...
# Generate the version control build info生成版本控制生成信息
...
# Generate the slave Dockerfile, and prepare build info for it生成从Dockerfile，并为它准备构建信息
...
# Add the versions in the tag, if the version change, need to rebuild the slave在标签中添加版本号，如果版本号改变，需要重新构建从机
